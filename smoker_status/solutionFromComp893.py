...
# ================== Initialize ==================
wandb.login(key="dfb04f00f29836b6832f472b7e052fb4861f3877")
...
train3 = pd.read_csv("smoker_status/train2.csv")  # <-- New dataset

# ================== Preprocessing ==================
def preprocess(train1, test1, train2, train3):
    for df in [train1, test1, train2, train3]:
    ...
    aligned_cols = train1.drop(['id', 'smoking'], axis=1).columns.tolist() + ['smoking']
    train2 = train2[aligned_cols]
    train3 = train3[aligned_cols]

    train1["is_train"] = train2["is_train"] = train3["is_train"] = 1
    test1["is_train"] = 0
    test1["smoking"] = -1

    full_df = pd.concat([train1, train2, train3, test1], axis=0, ignore_index=True)
...
# Apply preprocessing
train_df, test_df, test_ids = preprocess(train1, test1, train2, train3)
